...
const placaRegex = (tipo) => {
  ...
  if (!p) return /^$/; // aún no eligió tipo
...
export default function EditarVehiculo() {
  ...
  useEffect(() => {
    const obtener = async () => {
      try {
        ...
        // Mapear mayúsculas del backend → claves locales en minúscula
        const tipos = data.Tipos || "";
        const prefix = getPrefixByTipo(tipos);
        const placaSan = sanitizePlaca(data.Placa || "", prefix);
        ...
        setFormData({
          ...
          linea: data.Linea || "",
          modelo: data.Modelo ?? "",
          chasis: (data.Chasis || "").toUpperCase().slice(0, 20),
          color: data.Color || "",
          asientos: Number.isFinite(Number(data.Asientos)) ? Number(data.Asientos) : getAsientosByTipo(tipos),
          motor: data.Motor ?? "",
          combustible: data.Combustible || "",
          transmision: data.Transmision || "",
          impuesto_circulacion_anual: data.Impuesto_Circulacion_Anual ?? "",
          impuesto_anio_actual: data.Impuesto_Anio_Actual || "",
          kilometraje: data.Kilometraje ?? "",
          impresion_tarjeta_circulacion: data.Impresion_Tarjeta_Circulacion
            ? String(data.Impresion_Tarjeta_Circulacion).split("T")[0]
            : ""
        });
      } catch (err) {
  ...
  const handleTipoChange = (e) => {
    ...
    setFormData((prev) => ({
      ...
      marca: "" // forzar nueva selección acorde al tipo
    }));
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        Swal.fire("Error", err.error || "No se pudo actualizar el vehículo", "error");
      }
    } catch (error) {
  ...
  return (
    <SidebarLayout>
      <div className="container">
        <h2 className="mb-2">Editar Vehículo</h2>
